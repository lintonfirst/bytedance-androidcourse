...
public class NoteActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        final Integer extra = getIntent().getIntExtra("Id",-1);
        Log.d("Note",extra.toString());
        ...
        addBtn.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                if(extra<0){
                    ...
                    succeed=updateNode(extra)>0;
                }

                if (succeed) {
                    Toast.makeText(NoteActivity.this,
                            "Notes Update", Toast.LENGTH_SHORT).show();
    ...
    private int updateNode(Integer id) {
        if (database == null) {
            return -1;
        }
        ContentValues values = new ContentValues();
        values.put(TodoNote.COLUMN_CONTENT,editText.getText().toString().trim());
        values.put(TodoNote.COLUMN_PRIORITY, getSelectedPriority().intValue);
        values.put(TodoNote.COLUMN_STATE, State.TODO.intValue);
        values.put(TodoNote.COLUMN_DATE, System.currentTimeMillis());
